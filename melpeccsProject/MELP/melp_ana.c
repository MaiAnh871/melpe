...
//	if (rate == RATE1200)
	...
	lsf_vq(par);							//@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Bi Break o day tai frame 1@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	/*if (rate == RATE2400){
	...
	for (i = 0; i < NF; i++)
		quant_u(&par[i].jitter, &(quant_par.jit_index[i]), 0, MAX_JITTER_Q15, 2,
				ONE_Q15, 1, 7);
//	}
...
#pragma MUST_ITERATE(3,3,)
	for (section = 0; section < LPF_ORD/2; section++){
		for (i = (Shortword) (section << 1); i < (Shortword) ((section << 1) + 2);
			 i++){
			temp_delin[i] = sigbuf[LPF_ORD + FRAME - 1 - i + (section << 1)];
		}
		iir_2nd_s(&sigbuf[LPF_ORD], &lpf_den[section*3], &lpf_num[section*3],
				  &sigbuf[LPF_ORD], &lpfsp_delin[section << 1],
				  &lpfsp_delout[section << 1], FRAME);
		v_equ(&(temp_delout[section << 1]), &(lpfsp_delout[section << 1]), 2);
		iir_2nd_s(&sigbuf[LPF_ORD + FRAME], &lpf_den[section*3],
				  &lpf_num[section*3], &sigbuf[LPF_ORD + FRAME],
				  &lpfsp_delin[section << 1], &lpfsp_delout[section << 1],
				  PITCH_FR - FRAME);
		/* restore delay buffers for the next overlapping frame */
		v_equ(&(lpfsp_delin[section << 1]), &(temp_delin[section << 1]), 2);
		v_equ(&(lpfsp_delout[section << 1]), &(temp_delout[section << 1]), 2);
	}
	...
	fpitch[1] = find_pitch(&sigbuf[LPF_ORD + (PITCH_FR >> 1)], &dontcare,
						   (PITCHMIN << 1), PITCHMAX, PITCHMAX);
	...
	lpc_autocorr(&speech[(FRAME_END - (LPC_FRAME >> 1))], win_cof, auto_corr,
...
//	if (rate == RATE1200){
	...
	cur_track = (Shortword) (CUR_TRACK + (subnum << 1));
	for (i = 0; i < PIT_SUBNUM; i++) {
		pitchAuto(&speech[FRAME_END + i * PIT_SUBFRAME + (PIT_COR_LEN /2)],
				&pitTrack[cur_track + i + 1], &classStat[cur_track + i + 1]);
...
#pragma MUST_ITERATE(3,3,)
	for (i = 0; i < NF; i++){
		...
		/* ---- update silence energy ---- */
		if ((classStat[curTrack].classy == SILENCE) && (classStat[curTrack - 1].classy == SILENCE)){
