...
static void		corPeak(Shortword inbuf[], pitTrackParam * restrict pitTrack,
													classParam * restrict classStat);
...
static void lpfilt(Shortword inbuf[], Shortword lpbuf[], Shortword len)
{
	...
	for (i = 0; i < len; i++){
		...
		lpbuf[PIT_COR_LEN - len + i] = my_round(L_sum);                    /* Q0 */
	}
}
...
static void ivfilt(Shortword ivbuf[], Shortword lpbuf[], Shortword len)
{
	...
	r_coeff[0] = my_round(L_temp);						/* normalized r0 */
	for (i = 1; i < 3; i++){
		L40_sum = 0;
		for (j = i; j < PIT_COR_LEN; j++)
			L40_sum = L40_mac(L40_sum, lpbuf[j], lpbuf[j-i]);
		L_temp = (Longword) L40_shl(L40_sum, shift);
		r_coeff[i] = my_round(L_temp);
	}
	...
	for (i = 0; i < len; i++){
		...
		ivbuf[PIT_COR_LEN-len+i] = my_round(L_shl(L_temp, 3));
	}
}
