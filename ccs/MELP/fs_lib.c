...
void find_harm(Shortword * restrict input, Shortword * restrict fsmag, Shortword pitch,
...
#pragma MUST_ITERATE(6,10,)
	for (k = 0; k < num_harm; k++){
		...
		L_max = 0;
#pragma MUST_ITERATE(3,20,)
		for (j = 0; j < iwidth; j++){
			/*	temp1 = find_hbuf[2*(j + i)]; */
			/*	temp2 = find_hbuf[2*(j + i) + 1]; */
			temp2 = add(i, j);
			temp1 = find_hbuf[temp2 << 1];
			temp2 = find_hbuf[(temp2 << 1) + 1];
...
#pragma MUST_ITERATE(6,10,)
	for(k = 0; k < num_harm; k++)
		avg = _lsadd(L_fsmag[k],avg);	//avg = L40_add(avg, L_fsmag[k]);
