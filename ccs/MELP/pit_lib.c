...
Shortword f_pitch_scale(Shortword *restrict sig_out, Shortword * restrict sig_in,
						Shortword length)
{
					 ...
					 Shortword upper, Shortword length)
{
	...
	*pcorr = shr(sqrt_fxp(divide_s(extract_l(max_num), extract_l(max_denom)), 15), 1);
...
Shortword pitch_ana(Shortword * restrict speech, Shortword * restrict resid,
					...
					Shortword *pcorr2)
{
	...
	for (section = 0; section < LPF_ORD/2; section++){
		iir_2nd_s(&sigbuf[LPF_ORD_SOS], &lpf_den[section*3],
				  ...
				  &lpres_delin[section << 1], &lpres_delout[section << 1], FRAME);
		...
		iir_2nd_s(&sigbuf[LPF_ORD_SOS + FRAME], &lpf_den[section*3],
				  ...
				  &lpres_delin[section << 1], &lpres_delout[section << 1],
				  PITCH_FR - FRAME);
		/* restore delay buffers for the next overlapping frame */
		for (i = (Shortword) (section << 1); i < (Shortword) ((section << 1) + 2); i++){
	...
	temp = frac_pch(&sigbuf[LPF_ORD_SOS + (PITCH_FR >> 1)], &pcorr, pitch_est,
	...
	if (pcorr < PCORR_THR){
		...
		temp = frac_pch(&sigbuf[LPF_ORD + (PITCH_FR >> 1)], &pcorr, pitch_est,
		...
		if (pcorr < UVMAX) /* If correlation still too low, use average pitch */
			...
			if (temp > LONG_PITCH)                 /* longer pitches are more */
				...
				temp2 = PDOUBLE3;
			pitch = double_chk(&sigbuf[LPF_ORD + (PITCH_FR >> 1)], &pcorr,
