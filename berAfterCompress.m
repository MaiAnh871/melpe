%% Set input 
bitnum = 81;                                            % number of compressed bit
hex = 16;                                               % hexa
...
rs_bin_es = reshape (trp_bin_es, space, []);            % reshape into 96x849 = 96*5094/6
...
rs_binEr = reshape (binEr, bitnum, length(data)/row);   % reshape in to 81x849
...
rs_binOut = reshape (add2_binEr, hex, length(data));    % reshape into 16x5094
trp_binOut = rs_binOut.';                               % transpose into 5094x16
output = bi2de (trp_binOut, 'left-msb');                % convert binary to decimal

%% Calculate ber again
berO = nnz (bin_es - trp_binOut) / (length(data)/hex*bitnum);