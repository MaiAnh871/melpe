...
static float checkInv = 0; //check physical channel inversion on control packet
...
#ifdef writeBuff
	static int ProcessCtr(unsigned char* pkt,unsigned char* pkt2);	
#else
...
#endif
{
 ...
 //check for copies diff erence is less then a few bits
 ...
 if(!i)//packet is a voice data or errored
 {
  ...
  {
   for(int idx=0;idx<10;idx++) pkt2[idx]^=0xFF; //inverse 80 bits received data
   pkt2[10]^=1; //inverse bit 81
  }
  #endif
  return -3;
 }
...
#ifdef writeBuff
	int ProcessPkt(unsigned char* pkt, unsigned char* pkt2)
#else
...

{
 ...
 if(step>4) 
  #ifdef writeBuff
	  i=ProcessCtr(pkt,pkt2); //steps 5,6,7,8
  #else
	  i=ProcessCtr(pkt); //steps 5,6,7,8
  #endif
 
 else if(step>2) i=ProcessAU(pkt); //steps 3,4
 else i=ProcessDH(pkt); //steps 0,1,2
 if(i != -3) finv += checkInv; 
 return i;
}
