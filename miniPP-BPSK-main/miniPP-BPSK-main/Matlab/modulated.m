function [transmittSignal, outputBit] = modulated(inputBit)
    % Modulate data to signal to transmit
    % Ex: [transmittSignal, outputBit] = modulated(inputBit)
    %       inputBit: bitstream input, 81 bits for one frame  (12 buff each frame)
    %       outputBit: bitstream output include 9 parity bit
    ...
    N = length (inputBit);
    NoFrame = round(N/12);      % number of frame
    lastb = 0;                  % last bit
    ...
    for m=1:NoFrame
        ...
        parity(9) = 1;          
        buf = inputBit(12*(m-1)+1:12*m);    % input buff: each frame have 12 elements
        frameModulate = [];     % modulate each frame
        for i=1:10
            for j = 1:9
               if i==10         % add parity bit
                   bit = parity(j);
                   outputBit = [outputBit bit];
               else
                   k = 9*(j-1)+i;       % 
